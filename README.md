# NovaPlayer 🎬

<p align="center">
  <strong>一款支持竖屏上下滑动刷剧、纯粹轻量的跨平台局域网 (SMB) 与本地媒体播放器</strong>
</p>

<p align="center">
  <a href="https://github.com/guixun/NovaPlayer/releases/latest">
    <img src="https://img.shields.io/github/v/release/guixun/NovaPlayer?color=orange&label=Release" alt="Release">
  </a>
  <img src="https://img.shields.io/badge/Platform-Android-green.svg" alt="Platform">
  <img src="https://img.shields.io/badge/UX-Single--Hand%20Swipe-purple.svg" alt="UX">
  <img src="https://img.shields.io/badge/License-Proprietary-blue.svg" alt="License">
  <img src="https://img.shields.io/badge/AdFree-100%25-red.svg" alt="AdFree">
</p>

---

## 📖 简介 / Introduction

**NovaPlayer** 专为家庭影视发烧友、NAS 玩家、短剧追剧党以及追求极致单手操作体验的用户打造。

传统播放器几乎都只能“双手横屏握持”，看完一个视频还得退回目录重新找下一个，极其繁琐笨重。NovaPlayer 创新性地将**短视频信息流交互与 NAS 局域网媒体结合**：
👉 **像刷短视频一样上下滑动刷自己的本地和 NAS 视频，躺在床上单手大拇指就能刷完一整部剧！**

---

## ✨ 核心杀手级特性 / Killer Features

- 📱 **类短视频流上下滑动切片（独家亮点）**：
  打破传统播放器笨重的列表查找逻辑！上下滑动即可丝滑切换视频，单手单指即可完成切集、调音量、调亮度，是床上刷剧、看短视频集、刷手机 Vlog 的终极神器。
- ⚡ **原生 SMB 局域网极速直连**：
  原生支持 SMB / WebDAV / AList 协议，内置局域网 445 端口自动扫描探测，秒级连接你的 Windows 共享、群晖、威联通、极空间 NAS。
- 🎬 **短剧长片通吃，横竖自由**：
  既支持竖屏沉浸式单手刷短视频与生活 Vlog，也支持全屏横屏硬解 4K / HDR / 高码率影视大作，外挂字幕与多音轨一键切换。
- 🚫 **纯粹零干扰，无任何广告**：
  0 开屏广告、0 冗余短视频社交、0 后台流氓进程，极速秒开。
- 🪶 **单架构仅 21MB，超轻功耗**：
  纯净 arm64 架构精简打包，内存占用极低，长时间观影发热量显著低于同类产品。

---

## 📥 下载安装 / Download

### 最新稳定版：v1.0.1

| 下载渠道 | 下载方式 | 适用说明 |
| :--- | :--- | :--- |
| **国内高速镜像 (推荐)** | [👉 点击一键高速下载 APK](https://ghfast.top/https://github.com/guixun/NovaPlayer/releases/download/v1.0.1/app-release.apk) | 国内直连不限速，免翻免登录 |
| **GitHub 官方 Releases** | [👉 前往 GitHub Releases 页面](https://github.com/guixun/NovaPlayer/releases/latest) | 全球节点直接分发 |
| **夸克网盘 (备用推荐)** | [👉 前往夸克网盘备用下载](https://pan.quark.cn/s/61d224ef0bd7#/list/share) | 官方网盘高速下载通道 |

> ⚠️ **关于安卓手机“报毒 / 未知来源”拦截说明**：
> NovaPlayer 为个人独立开发应用，目前尚未上架各大品牌应用市场。部分国产手机（华为、小米、OPPO、vivo 等）在安装时若弹出“未经安全检测 / 未知来源应用”等提示，属于系统的常规防御机制。请选择**“继续安装 / 已知晓风险”**即可，本应用保证无任何后门与侵权行为。

---

## 💬 社区交流与反馈 / Community & Support

NovaPlayer 正在持续快速迭代中，欢迎所有尝鲜玩家加入讨论群，向我们提交宝贵的体验建议与 Bug 报修：

- 🐧 **官方交流 QQ 群**：**818733351**（欢迎进群交流 NAS 玩法、提出新功能需求）
- 📮 **腾讯兔小巢反馈专页**：[点击前往兔小巢提交建议](https://support.qq.com/product/802090)（微信一键免登录）
- 🐞 **GitHub Issues**：[提交 Bug 报修 / Feature Request](https://github.com/guixun/NovaPlayer/issues)

---

## ☕ 支持开发者 / Sponsor

NovaPlayer 是由独立开发者在业余时间完全自主维护的工具。如果它改善了你的局域网家庭观影体验，欢迎请作者喝杯咖啡 ☕，你的暖心支持是项目持续迭代与优化的最大动力！

<details>
<summary>👉 <b>点击展开微信赞赏码 (WeChat Sponsor)</b></summary>
<br>
<p align="center">
  <img src="assets/wechat_sponsor.png" width="220" alt="NovaPlayer 微信赞赏码">
  <br>
  <sub>截图后可在微信「扫一扫」中从相册选取识别 · 感谢支持！</sub>
</p>
</details>

---

## 🔒 隐私与免责声明 / Privacy & Disclaimer

1. **隐私安全**：NovaPlayer 尊重每一位用户的隐私权。应用不会收集你的通讯录、短信或任何私人敏感文件；仅在冷启动时收集匿名的随机设备代号与应用版本号，用于统计日活与版本更新，不涉及任何个人身份标识。
2. **协议免责**：本软件仅作为本地与局域网存储的浏览与播放工具，不提供任何音视频资源，用户请遵守相关法律法规。
